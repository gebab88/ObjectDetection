...
Detection_OpenVINO::Detection_OpenVINO( float score_threshold,
    ...
    inter_request_       = compiled.create_infer_request();
...
void Detection_OpenVINO::detect( Mat &frame) {
    ...
    inter_request_.set_input_tensor(input_);

    // ── Inferenz ──────────────────────────────────────────────────────
    inter_request_.infer();

    output_ = inter_request_.get_output_tensor(0);
